...
export default function CourseClient({ course, courseId }: CourseClientProps) {
  ...
  const { isEnrolled, enrolledCourses } = useAppSelector((state) => {
    const enrolled = selectIsEnrolled(state, courseId);
    return {
      isEnrolled: enrolled,
      enrolledCourses: state.myCourses.enrolledCourses,
    };
  });
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
              ...
              <div className="lg:col-span-1">
                <div className="relative aspect-video rounded-xl overflow-hidden shadow-2xl">
                  {course.thumbnailUrl ? (
                    <Image width={40} height={40} src={course.thumbnailUrl} alt={course.title} className="w-full h-full object-cover" />
                  ) : (
